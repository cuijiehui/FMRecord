...
/**
 * author : created by cui on 2019/10/9 18:43
 */
public class LyricView extends View {
    ...
    // 高亮歌词
    private static final int DEFAULT_COLOR_FOR_HIGHLIGHT_LRC = 0xff000000;
    // 高亮歌词上下一句
    private static final int DEFAULT_COLOR_BESIDE_HIGHLIGHT_LRC = 0x50000000;
    // 其他歌词
    private static final int DEFAULT_COLOR_FOR_OTHER_LRC = 0x30000000;//没歌曲的颜色
    private static final int DEFAULT_COLOR_FOR_PROGRESS =0x55000000;
    private static final int COLOR_FOR_TIME_LINE = 0xff000000;
    // 自动滚动每一行歌词的持续时长
    ...
    private ArrayList<String> lrcRowList = new ArrayList<>();
    // 实现歌词垂直方向滚动的辅助类
...
//        }
        ...
        for (int i = 0; i < lrcRowList.size(); i++) {
            String lrc = getLrc(i);
//            if (i == curLine) {
            Log.d("lyricview","lrc="+lrc);
            int count = drawHighlightText(canvas, lrc, y);
//            } else {
//                drawNormalText(canvas, i, y);
//            }
            // 计算得到y坐标
            y = y + (eachLineHeight*(count+1));
        }
    }
    ...
    public ArrayList<String> getLrcRowList() {
        return lrcRowList;
    }

    public void setLrcRowList(ArrayList<String> lrcRowList) {
...
//        canvas.drawBitmap(playBitmap, x-57, y - playBitmap.getHeight()/2, null);
    ...
    private int drawHighlightText(Canvas canvas, String text, float y) {
        int count = 0;
        if (text.isEmpty()) {
            return count;
        }
        ...
        if (textWidth > LRC_MAX_WIDTH) {
            ...
            for (int i = 0; i < textChars.length; i++) {
                ...
                float textCharsWidth = highlightPaint.measureText(textBuffer.toString()+textChar);
                if (textCharsWidth<=LRC_MAX_WIDTH) {
                    textBuffer.append(textChar);
                    if (i==textChars.length-1) {
                        x=(getWidth() -  highlightPaint.measureText(textBuffer.toString())) / 2;
                        canvas.drawText(textBuffer.toString(), x, y, highlightPaint);
                    }
                }else{
                    x=(getWidth() -  highlightPaint.measureText(textBuffer.toString())) / 2;
                    Log.d("Lyric","x="+x);
            ...
            Log.d("lyricview","textBuffer.toString()="+text);
...
//                event.getY() < (y + playBitmap.getHeight() + spaceHolder);
    ...
    public void setLrcRows(ArrayList<String> lrcRows) {
        reset();
        isLoadingLrc = false;
