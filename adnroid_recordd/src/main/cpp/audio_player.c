...
bool IS_PLAYAUDIO =true;

JNIEXPORT void JNICALL Java_com_example_ffmpegdemo2_utils_AudioPlayer_stop
(JNIEnv *env, jobject jthiz){
    IS_PLAYAUDIO=false;
}


JNIEXPORT void JNICALL Java_com_example_recorddemo_ffmpeg_Decode_decode(
		JNIEnv *env,jobject jthiz,jstring input_jstr,jstring output_path
		){
	const char* input_cstr = (*env)->GetStringUTFChars(env,input_jstr,NULL);
	FILE *fd = fopen(output_path,"rw");
	...
	//不断读取编码数据
	while(av_read_frame(pFormatCtx,packet) >= 0&&IS_PLAYAUDIO){
		//解码音频类型的Packet
...
JNIEXPORT void JNICALL Java_com_example_ffmpegdemo2_utils_AudioPlayer_play
